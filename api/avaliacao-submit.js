...
const INSTRUMENTOS = {
  ...
  "CDE": {
    ...
    valMin: 1,
    valMax: 6,
    // item 9 (0-indexado: 8) = "já ameacei me machucar" → autoagressão, trata como risco
    itemCriticoIndex: 8,
    // faixas por percentil da amostra original colombiana (Lemos & Londoño, 2006)
    faixas: [
      [23, 46, "baixo"],
      [47, 70, "médio"],
      [71, 88, "elevado"],
      [89, 138, "muito elevado"],
    ],
  },
};
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    for (const id of t.instrumentos || []) {
      ...
      const vMin = cfg.valMin != null ? cfg.valMin : 0;
      const vMax = cfg.valMax != null ? cfg.valMax : 3;
      if (limpo.some((n) => !Number.isInteger(n) || n < vMin || n > vMax)) {
      ...
      // limiar de risco por instrumento: PHQ-9 (escala 0-3) qualquer valor > 0;
      // CDE (escala 1-6, mínimo é 1) considera risco a partir de 2, ou seja,
      // qualquer coisa acima do "completamente falso pra mim".
      const limiarRisco = cfg.valMin != null && cfg.valMin > 0 ? cfg.valMin : 0;
      const itemCritico =
        cfg.itemCriticoIndex != null &&
        limpo[cfg.itemCriticoIndex] > limiarRisco;
