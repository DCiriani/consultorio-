...
function Painel({pacientes,setPacientes,registros,setRegistros,titulares,setTitulares,onCadastro,onLogout}){
  ...
  async function salvarEdicaoReg(id,dados){
    ...
    setRegistros(registros.map(r=>r.id===id?{...r,...dados}:r));
    setEditandoReg(null);
    showT("Pagamento atualizado.");
  }

  async function salvarEdicaoReg(){
    if(!editandoReg.pagamento){showT("Selecione a forma de pagamento.","erro");return;}
    await updateItem("reg",editandoReg.id,{data:editandoReg.data,pagamento:editandoReg.pagamento,valor:editandoReg.valor});
    const a=registros.map(r=>r.id===editandoReg.id?editandoReg:r);
    setRegistros(a);
    setEditandoReg(null);
    showT("Pagamento atualizado!");
  }
  ...
  return(
    <div style={ROOT}>
      <Toast t={toast}/>
      {detalhe&&<ModalFicha p={detalhe} titulares={titulares} registros={registros} onClose={()=>setDetalhe(null)}/>}
      ...
      {editandoReg&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={()=>setEditandoReg(null)}>
        <div style={{background:"#fff",borderRadius:14,padding:28,width:"100%",maxWidth:420,boxShadow:"0 8px 40px rgba(0,0,0,0.2)"}} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px 14px",marginBottom:18}}>
            <div>
              <label style={LBS}>Data</label>
              <input style={INS} value={editandoReg.data} onChange={e=>setEditandoReg({...editandoReg,data:e.target.value})}/>
            </div>
            <div>
              <label style={LBS}>Valor (R$)</label>
              <input style={INS} value={editandoReg.valor==="—"?"":editandoReg.valor} onChange={e=>setEditandoReg({...editandoReg,valor:e.target.value||"—"})} placeholder="ex: 200,00"/>
          ...
          <button onClick={salvarEdicaoReg} style={{width:"100%",padding:13,background:"#2a7a4a",color:"#fff",border:"none",borderRadius:9,fontSize:15,fontWeight:700,cursor:"pointer",fontFamily:"sans-serif"}}>✓ Salvar alterações</button>
        </div>
      </div>}
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontFamily:"sans-serif"}}>
              ...
              <tbody>{registros.slice(0,20).map(r=><tr key={r.id}>
                ...
                <td style={{padding:"9px 12px",borderBottom:"1px solid #eef4ec",whiteSpace:"nowrap"}}>
                  ...
                  <button onClick={()=>excluirReg(r.id)} style={{background:"none",border:"none",color:"#c0392b",cursor:"pointer",fontSize:14}} title="Excluir">✕</button>
                </td>
              </tr>)}</tbody>
            </table>
          </div>
